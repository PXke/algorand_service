...
class ArticleStmts:
    ...
    EXISTS = _Stmt("SELECT article_id FROM articles_by_id WHERE article_id = ?")
    GET_TAGS = _Stmt("SELECT tags FROM articles_by_id WHERE article_id = ?")
    GET_PUBLISHED_AT = _Stmt("SELECT published_at FROM articles_by_id WHERE article_id = ?")
    GET_IMAGE_META = _Stmt(
        "SELECT service_id, source_url, image_url FROM articles_by_id WHERE article_id = ?"
    )
    GET_IMAGE = _Stmt("SELECT image_url FROM articles_by_id WHERE article_id = ?")
    GET_FOR_FEED = _Stmt(
    ...
    UPDATE_IMAGE = _Stmt("UPDATE articles_by_id SET image_url = ? WHERE article_id = ?")


class FeedStmts:
    BY_BUCKET = _Stmt(
        "SELECT article_id, service_id, title, summary, published_at "
        "FROM articles_feed WHERE bucket = ? LIMIT ?"
    )
    BY_BUCKET_TAGS = _Stmt(
        "SELECT published_at, tags FROM articles_feed WHERE bucket = ? LIMIT ?"
    )
    BY_BUCKET_RECENT = _Stmt(
        "SELECT article_id, title, tags, published_at FROM articles_feed WHERE bucket = ?"
    )
    INSERT = _Stmt(
    ...
    UPDATE_IMAGE = _Stmt(
        "UPDATE articles_feed SET image_url = ? "
...
class PublishQueueStmts:
    DEDUPE_GET = _Stmt(
        "SELECT queue_id FROM publish_queue_dedupe WHERE dedupe_key = ?"
    )
    INSERT = _Stmt(
    ...
    LIST_PENDING = _Stmt(
        ...
        "FROM publish_queue_pending WHERE status = ? LIMIT ?"
    )
    GET_DETAIL = _Stmt(
        "SELECT display_name, scrape_url, payload, created_at "
        "FROM publish_queue WHERE queue_id = ?"
    )
    COUNT_PENDING = _Stmt(
        "SELECT COUNT(*) AS n FROM publish_queue_pending WHERE status = ?"
    )
    ...
    UPDATE_STATUS = _Stmt(
        "UPDATE publish_queue SET status = ?, updated_at = ? WHERE queue_id = ?"
    )
    ...
    DELETE_DEDUPE = _Stmt(
        "DELETE FROM publish_queue_dedupe WHERE dedupe_key = ?"
    )
...
class UrlQueueStmts:
    BY_URL = _Stmt(
        "SELECT queue_id, status FROM url_queue_by_url WHERE url = ?"
    )
    GET_STATUS = _Stmt("SELECT status FROM url_queue WHERE queue_id = ?")
    INSERT = _Stmt(
    ...
    INSERT_BY_URL = _Stmt(
        "INSERT INTO url_queue_by_url (url, queue_id, enqueued_at) VALUES (?, ?, ?)"
    )
    INSERT_PENDING = _Stmt(
    ...
    PEEK_PENDING = _Stmt(
        ...
        "FROM url_queue_pending WHERE status = ? LIMIT 1"
    )
    UPDATE_STATUS = _Stmt("UPDATE url_queue SET status = ? WHERE queue_id = ?")
    DELETE_PENDING = _Stmt(
        "DELETE FROM url_queue_pending "
        "WHERE status = ? AND priority = ? AND enqueued_at = ? AND queue_id = ?"
    )
    GET_METADATA = _Stmt("SELECT metadata FROM url_queue WHERE queue_id = ?")
    LIST_PENDING_IDS = _Stmt(
        "SELECT queue_id FROM url_queue_pending WHERE status = ? LIMIT 10000"
    )
...
class ClassifierReviewStmts:
    ...
    COUNT_PENDING = _Stmt(
        "SELECT review_id FROM classifier_review_pending WHERE status = ? LIMIT ?"
    )
    LIST_PENDING_URLS = _Stmt(
        "SELECT url FROM classifier_review_pending WHERE status = ? LIMIT ?"
    )
    LIST_PENDING = _Stmt(
...
class DomainTrackingStmts:
    GET_STATUS = _Stmt(
        ...
        "FROM domain_tracking WHERE domain = ?"
    )
    ...
    LIST = _Stmt(
        ...
        "FROM domain_tracking LIMIT ?"
    )
    UPDATE_METADATA = _Stmt(
        "UPDATE domain_tracking SET metadata = ? WHERE domain = ?"
    )
    GET_METADATA = _Stmt(
        "SELECT metadata FROM domain_tracking WHERE domain = ?"
    )
...
class ClassifierFeedbackStmts:
    ...
    LIST_IDS = _Stmt(
        "SELECT feedback_id FROM classifier_feedback_by_time WHERE bucket = ? LIMIT ?"
    )
    GET = _Stmt(
        "SELECT url, text_sample, category, predicted_category, quality, approved "
        "FROM classifier_feedback WHERE feedback_id = ?"
    )
    GET_GRADE = _Stmt(
...
class PriceMetricsStmts:
    ...
    LIST_SAMPLES = _Stmt(
        ...
        "change_24h_pct, market_cap_usd, volume_24h_usd, source "
        "FROM price_metric_samples WHERE asset_id = ? AND collected_at >= ? LIMIT ?"
    )
    INSERT_BRIEF = _Stmt(
    ...
    GET_BRIEF = _Stmt(
        ...
        "change_24h_pct, sample_count_24h, sample_count_7d, mistral_context "
        "FROM price_metrics_brief WHERE asset_id = ?"
    )
...
class SnapshotStmts:
    GET_LATEST = _Stmt(
        "SELECT content_hash, title, body FROM page_snapshots "
...
class ArticleMatchStmts:
    FIND_BY_KEY = _Stmt(
        "SELECT article_id, edit_window_closes_at FROM article_match_keys "
...
class IntelligenceStmts:
    GET = _Stmt(
        ...
        "FROM service_intelligence WHERE service_id = ?"
    )
    GET_FIRST_SEEN = _Stmt(
        "SELECT first_seen_at FROM service_intelligence WHERE service_id = ?"
    )
    INSERT = _Stmt(
...
class ToolInsightStmts:
    ...
    BUMP_USAGE = _Stmt(
        "UPDATE tool_usage_stats SET calls = calls + ?, errors = errors + ? "
        "WHERE day = ? AND tool = ?"
    )
...
class ServiceProfileStmts:
    GET_WEIGHT = _Stmt(
        "SELECT impressiveness_score FROM service_profiles WHERE service_id = ?"
    )
    INSERT = _Stmt(
...
class PendingFeedStmts:
    PEEK = _Stmt(
        ...
        "FROM pending_feed_queue WHERE bucket = ? LIMIT 1"
    )
    DELETE = _Stmt(
        "DELETE FROM pending_feed_queue "
        "WHERE bucket = ? AND interest_score = ? AND approved_at = ? AND article_id = ?"
    )
    PEEK_ID = _Stmt(
        "SELECT article_id FROM pending_feed_queue WHERE bucket = ? LIMIT 1"
    )
...
class ViewCountStmts:
    GET_BULK = _Stmt(
        "SELECT article_id, views FROM article_view_counts WHERE article_id IN ?"
    )
...
class CrawlerConfigStmts:
    LIST_ALL = _Stmt(
        "SELECT crawler_type, display_name, description, enabled FROM crawler_config"
    )
...
class OfficialChannelStmts:
    BY_KIND = _Stmt(
        "SELECT channel_id FROM official_channels WHERE kind = ?"
    )
