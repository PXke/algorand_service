...
# above, re-exported here) since both services need it: workers' own
# admin_source_store.load_active_sources reads it (projected into a
# recompose's prompt + trace); backend's admin API writes it. See that
# module's own docstring for why LIST_ACTIVE_BY_ARTICLE doesn't filter
# `status` in CQL.
# --------------------------------------------------------------------------- #
