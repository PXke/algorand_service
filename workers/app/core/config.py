...
# https://docs.x.com/x-api/getting-started/pricing). Shipped 2026-08-21 as a
# live per-compose writer tool call; briefly redesigned 2026-08-25 into a
# weekly scheduled sweep + cache read (to stop per-compose spend), then
# reverted 2026-08-28 back to live per-compose calls (owner call: the
# composer should be able to query X live within a capped daily budget,
# rather than being confined to a fixed list of tracked services -- a real
# story about an untracked project, e.g. Lumi Rogue, got nothing from the
# cache because that project's weekly sweep hadn't run yet). Unlike
# Bluesky, this is real money per call, so it's opt-in (default off) and
# kept deliberately small: research_tools.py fixes max_results at 10 (X's
# own API minimum, and the smallest unit of cost) rather than letting the
# model request more, and X_SEARCH_DAILY_CAP hard-stops total calls per UTC
# day via a Redis counter (same INCR+EXPIRE pattern as
# publish_daily_guard.py) -- refusing outright once hit, not degrading
# quality. Session-level throttling lives alongside suggest_tool/
# suggest_glossary_term in llm_tool_loop.py's CALL_CAPPED_TOOLS so one
# article can't burn a large slice of the daily budget alone.
