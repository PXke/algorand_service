"""Extract OG/meta tags, hero image, and publish date from a fetched page."""
...
def extract_source_logo(soup: BeautifulSoup, base_url: str = "") -> str:
...
def is_stale_page(published_at: str, max_age_days: int) -> bool:
    """True only when we have a date AND it's older than the window. No date => not stale (don't penalise pages that simply omit a publish date)."""
