"""Celery entry for the periodic service-duplication reconciliation sweep (see `app.modules.newspaper.service_reconciliation` for the two scans this runs and exactly what counts as a safe, deterministic auto-action versus a flagged-for-review one).
...
venue_service_id). Safe to run unattended in prod -- nothing ambiguous is
ever auto-merged or auto-backfilled, see that module's own docstrings.
"""
...
def reconcile_service_duplicates() -> dict[str, object]:
    """Daily beat: run both service-duplication reconciliation scans and return their combined findings/actions."""
