"""Fires social distribution (Bluesky, Telegram, ...) after an article goes
live — called from every path that publishes genuinely NEW content (fresh
auto-publish, the daily queue-drain release, and admin-approved publishes
triggered from the backend). Deliberately NOT called from recompose
auto-apply — reposting every refresh of already-published content would look
repetitive to followers. Always uses the generated share-card image
(/og/article/{id}.png), not whatever "real" image_url the article happens
to have — guarantees a consistent branded card on every channel instead of
sometimes falling back to a source's low-res favicon.
"""
