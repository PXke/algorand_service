...
def compose_weekly_price_article(snapshot: WeeklyPriceSnapshot) -> tuple[str, str, str]:
    direction = "up" if snapshot.week_change_pct >= 0 else "down"
    title = f"Weekly {snapshot.asset_name} ({snapshot.currency}) price analysis"
    summary = (
        f"{snapshot.asset_name} is {direction} {abs(snapshot.week_change_pct):.2f}% over the "
        f"past 7 days; last price ${snapshot.price_usd:,.4f} {snapshot.currency}."
    )
    week_label = snapshot.as_of.strftime("%Y-%m-%d")
    body = "\n".join(
        [
            f"# {snapshot.asset_name} — 7-day price snapshot",
            "",
            f"*As of {snapshot.as_of.strftime('%Y-%m-%d %H:%M UTC')}*",
            "",
            "| Metric | Value |",
            "|--------|-------|",
            f"| Current | ${snapshot.price_usd:,.4f} |",
            f"| 7d open | ${snapshot.week_open_usd:,.4f} |",
            f"| 7d high | ${snapshot.week_high_usd:,.4f} |",
            f"| 7d low | ${snapshot.week_low_usd:,.4f} |",
            f"| 7d change | {snapshot.week_change_pct:+.2f}% |",
            "",
            "## Summary",
            "",
            (
                f"Over the trailing 7 days, **{snapshot.asset_name}** moved "
                f"**{snapshot.week_change_pct:+.2f}%** from "
                f"${snapshot.week_open_usd:,.4f} to ${snapshot.price_usd:,.4f}. "
                f"The weekly range was ${snapshot.week_low_usd:,.4f} - "
                f"${snapshot.week_high_usd:,.4f}."
            ),
            "",
            f"_Week label: {week_label}. Data: CoinGecko market chart._",
        ]
    )
