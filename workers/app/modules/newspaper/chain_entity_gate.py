...
def _lookup_status(kind: str, value: str) -> str:
    ...
    'missing' requires clean 404s from BOTH networks; any transport/API error
    downgrades to 'unknown' so an outage can never condemn a real citation.
    """
    ...
    if kind == "asset":
        main = _classify(_algod_get(f"/v2/assets/{value}"))
        if main == "found":
            return "mainnet"
        test = _classify(_testnet_idx_get(f"/v2/assets/{value}"))
        if test == "found":
            return "testnet"
    elif kind == "txid":
        main = _classify(_mainnet_idx_get(f"/v2/transactions/{value}"))
        if main == "found":
            return "mainnet"
        test = _classify(_testnet_idx_get(f"/v2/transactions/{value}"))
        if test == "found":
            return "testnet"
    elif kind == "address":
...
def link_and_verify_chain_entities(
    ...
    *,
    ...
    # 1) Delink explorer urls whose target entity is provably missing (or an
    #    invalid address) — same delink-not-rewrite rule as the link gate.
    def _delink(match: re.Match) -> str:
        ...
        if status in ("missing", "invalid"):
            # Audit entry comes from the statuses loop below (which covers
            # every entity, explorer-linked or not) — appending here too would
            # double-count.
            return match.group(1)
        return match.group(0)

    body = _MD_LINK_RE.sub(_delink, body)

    # 2) Auto-link the first plain-text occurrence of each verified entity.
