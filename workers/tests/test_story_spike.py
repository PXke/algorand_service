"""abort_article tool (2026-07-17): lets the writer refuse to compose a story
...
def test_chat_with_tools_reraises_spike_and_records_trace(monkeypatch) -> None:
    ...
    article' — abort_article is the deliberate exception. It must escape the
...
def test_no_spike_needed_reason_does_not_abort() -> None:
    """The writer misused abort_article to narrate 'No spike needed' and threw away
...
def test_genuine_spike_still_aborts() -> None:
    ...
    with pytest.raises(StorySpikedError):
        abort_article_handler(category="dead_project",
