...
def test_classify_foundation_discord_warning_as_scam_breaking(
...
def test_breaking_tier_disabled_by_default_even_for_scam_topic() -> None:
    """Regression pin (2026-07-17): a crypto.news interview boasting 'zero downtime' got keyword-matched into NETWORK_INCIDENT and shipped as 'Breaking:' about a 10-month-old, already-concluded campaign. Owner decision: no deterministic BREAKING tier — a future version should have the WRITER decide via an explicit tool call, not a keyword scan. Even a genuine SCAM_ALERT topic must not escalate to BREAKING while this is off."""
    text = (
        "@everyone WARNING DO NOT interact with algoblow.com! "
        "This is a malicious app that will attempt to take control of accounts "
        "by including a rekey in transaction requests."
