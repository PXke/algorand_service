"""Discovery-lane widening (2026-07-09): link-following + curated lists miss
services nobody links or curates. Four new inputs: (1) on-chain asset-config
url params (the chain declares domains — HAFN carries hesab.af), (2) DefiLlama
/ Pera-verified API registries, (3) GitHub-topic + Medium-tag + Bluesky-link
mentions, (4) a retro-pass promoting pending domains on crawled-content score."""
...
def _acfg_tx(au: str) -> RoundTransaction:
    return RoundTransaction(
        txid="T", round=1, sender="SND", txn_type="acfg",
...
def test_non_acfg_txn_unaffected() -> None:
    tx = RoundTransaction(
        txid="T", round=1, sender="SND", txn_type="pay",
...
def test_defillama_filters_to_algorand_non_cex(monkeypatch) -> None:
    protocols = [
        {"name": "Binance CEX", "category": "CEX", "chains": ["Algorand"],
         "url": "https://www.binance.com", "slug": "binance"},
        {"name": "Folks Finance", "category": "Lending",
         "chains": ["Algorand", "Avalanche"],
         "url": "https://folks.finance", "slug": "folks-finance"},
        {"name": "Uniswap", "category": "Dexes", "chains": ["Ethereum"],
         "url": "https://uniswap.org", "slug": "uniswap"},
    ]
    monkeypatch.setattr(
        "app.core.net_guard.guarded_get",
        lambda url, **kw: SimpleNamespace(
            json=lambda: protocols, raise_for_status=lambda: None
        ),
    )
...
def test_pera_verified_resolves_asset_urls(monkeypatch) -> None:
    monkeypatch.setattr(
        "app.core.net_guard.guarded_get",
        lambda url, **kw: SimpleNamespace(
            json=lambda: {"results": [
...
def test_reevaluate_promotes_only_scored_pending_above_threshold(monkeypatch) -> None:
    import app.modules.crawler.tasks.url_queue_tasks as uq

    rows = [
        SimpleNamespace(domain="good.fi", frontier_status="pending", is_relevant=True,
                        metadata={"content_relevance": "0.61"}),
        SimpleNamespace(domain="lowscore.io", frontier_status="pending", is_relevant=True,
                        metadata={"content_relevance": "0.12"}),
        SimpleNamespace(domain="unscored.dev", frontier_status="pending", is_relevant=True,
                        metadata={}),
        SimpleNamespace(domain="already.app", frontier_status="approved", is_relevant=True,
                        metadata={"content_relevance": "0.9"}),
        SimpleNamespace(domain="rejected.com", frontier_status="pending", is_relevant=False,
                        metadata={"content_relevance": "0.9"}),
    ]
    monkeypatch.setattr(
        "app.core.cassandra.get_cassandra_session",
        lambda: SimpleNamespace(
            execute=lambda stmt, params: rows,
            prepare=lambda cql: cql,  # _Stmt descriptor prepares on first access
        ),
    )
    monkeypatch.setattr(
        uq, "classify_pending_domains",
        lambda **kw: {"scored": 0, "errors": 0, "unreadable": 0},
    )
    promoted = []
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda url, **kw: enqueued.append(url) or ("", True),
    )
    out = uq.reevaluate_pending_domains(limit=10)
    assert out["promoted"] == 1 and out["promoted_domains"] == ["good.fi"]
