...
def test_article_not_found_retires_queue_row(monkeypatch) -> None:
    """Audit 2026-07-17: a deleted linked article is a PERMANENT condition,
    but the plain "skipped" status left the queue row pending — redrained
    every ~2-minute breaking beat forever and starving the service's one
    pending-row slot. queue_status lets _resolve retire the row."""
    import app.modules.newspaper.article_edit_service as svc

    monkeypatch.setattr(svc, "get_article", lambda article_id: None)
...
def test_edit_reregisters_match_keys_anchored_to_publish_time(monkeypatch) -> None:
    """Audit 2026-07-17: re-registering match keys with the default
    closes_at (now + window) meant every edit rolled the article's edit
    window forward another 24h — and since each edit also adds the editing
    source's own keys, one stray match could keep an article editable (and
    accumulating keys) indefinitely. That rolling window is how the runaway
    loop kept re-opening itself. Keys must re-register anchored to the
    article's ORIGINAL publish time, converging with is_edit_window_open."""
    ...
    monkeypatch.setattr(
        "app.modules.newspaper.indexnow.ping_article", lambda *a, **k: None
    )
    registered: dict = {}
    monkeypatch.setattr(
    ...
    expected = edit_window_closes_at(
        from_time=datetime.fromtimestamp(published_epoch, tz=UTC)
    )
    assert registered["closes_at"] == expected


def test_stale_edit_row_falls_through_to_create_when_window_closed(monkeypatch) -> None:
    """Audit 2026-07-17: publish_mode is decided at INGEST time, but a row
    can sit pending for days behind cooldowns (observed: 4-day-old edit row)
    and drain long after the linked article's edit window closed. The drain
    must re-check the window: closed -> compose as a NEW article (what
    resolve_publish_mode would decide today), never edit a days-old piece."""
    import pytest
...
def test_open_window_edit_row_still_edits(monkeypatch) -> None:
    from app.modules.newspaper.tasks import publish_tasks as pt
