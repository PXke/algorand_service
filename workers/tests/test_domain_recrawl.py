"""should_recrawl_domain: admin rejects are permanent; rejected-while-pending
domains must not be crawled (the bug where they kept getting crawled)."""
...
def _patch_status(monkeypatch, status):
    monkeypatch.setattr(dt, "get_domain_status", lambda domain: status)


def test_unknown_domain_allowed(monkeypatch):
    _patch_status(monkeypatch, None)
    assert dt.should_recrawl_domain("x.com") is True


def test_relevant_domain_allowed(monkeypatch):
    _patch_status(monkeypatch, {"is_relevant": True})
    assert dt.should_recrawl_domain("x.com") is True


def test_admin_reject_is_permanent(monkeypatch):
...
def test_rejected_while_pending_not_crawled(monkeypatch):
...
def test_auto_irrelevant_rechecks_after_window(monkeypatch):
...
def test_blocklisted_is_dead_end_without_db_read(monkeypatch):
    calls = {"n": 0}

    def _counting_status(domain):
        calls["n"] += 1
        return None
...
def test_evaluate_reads_status_once(monkeypatch):
    calls = {"n": 0}
...
def test_evaluate_states(monkeypatch):
    _patch_status(monkeypatch, None)
...
def test_protected_excludes_english_false_positives():
