"""Anchor pairing: human tags -> AnnotatedSample, and (human, machine) pair construction feeding the validation harness."""
...
def test_human_sample_from_tags() -> None:
    """Builds an AnnotatedSample from human review tags, preserving flags and error types."""
    h = human_sample_from_tags(
        {"factuality_fail": True, "tone_fail": False, "error_types": ["numeric_drift"]}
    )
    assert h.factuality_fail
    assert not h.tone_fail
    assert h.error_types == ("numeric_drift",)
    assert h.source == "anchor"


def test_human_sample_defaults() -> None:
    """Defaults to no failures and no error types when given empty tags."""
    h = human_sample_from_tags({})
    assert not h.factuality_fail
    assert not h.tone_fail
    assert h.error_types == ()


def test_build_pair_runs_machine_annotator() -> None:
    """Runs the Tier-1 machine annotator and confirms it independently agrees with the human tag."""
    ...
    assert human.factuality_fail
    assert machine.factuality_fail
    assert "unsupported_elaboration" in machine.error_types


def test_pairs_feed_validation_harness() -> None:
    """A batch of anchor pairs where the annotator agrees with humans validates ungated."""
