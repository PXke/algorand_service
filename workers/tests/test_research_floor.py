"""Stage-1 research floor: count distinct research SOURCES (domains fetched, or
a stable per-tool identity when a call carries no URL), excluding review_draft.
...
def test_same_domain_via_different_tools_counts_once() -> None:
    trace = [
        {"tool": "search_web", "arguments": {"query": "algorand"},
         "result": {"results": [{"url": "https://forum.algorand.org/t/1"}]}},
        {"tool": "fetch_url", "arguments": {"url": "https://forum.algorand.org/t/1"},
...
def test_one_search_call_surfacing_many_domains_counts_each() -> None:
    trace = [
        {"tool": "search_web", "arguments": {"query": "algorand defi"},
         "result": {"results": [
...
def test_six_trivial_calls_no_longer_gamed_without_breadth() -> None:
    ...
    trace = [
        {"tool": f"tool_{i}", "arguments": {"url": "https://same-domain.example/x"},
