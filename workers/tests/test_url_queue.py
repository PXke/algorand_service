...
def test_enqueue_url_deduplicates_pending(monkeypatch, fake_cassandra_session) -> None:
...
def test_enqueue_url_inserts_new(monkeypatch, fake_cassandra_session) -> None:
    monkeypatch.setattr("app.core.config.URL_QUEUE_ENABLED", True)
...
def test_dequeue_url_returns_highest_priority_item(monkeypatch, fake_cassandra_session) -> None:
    monkeypatch.setattr("app.core.config.URL_QUEUE_ENABLED", True)
    pending = MagicMock(
        queue_id="q-high",
...
def test_mark_url_done_updates_status(fake_cassandra_session) -> None:
