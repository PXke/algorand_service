"""SPA (browser://) sources must classify as web so the per-domain daily cap and diversity cooldown apply — otherwise they silently bypass both."""
...
def test_browser_url_classifies_as_web() -> None:
    """Classifies browser:// and other scheme-prefixed URLs by their real source kind (web/youtube/mail)."""
...
def test_domain_from_browser_url_resolves_real_host() -> None:
    # Regression: used to return "https" for browser://https://… , giving every
    # SPA source one shared bogus cooldown bucket.
    """Resolves a browser:// URL's registrable domain from its wrapped real host, not the literal string "https"."""
