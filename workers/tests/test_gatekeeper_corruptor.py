"""Corruptor mutations: anchor-aware drift with a tolerance band, hard negatives,
and the symmetric-paraphraser discipline that prevents fingerprint shortcuts."""
...
def _gold() -> Sample:
    return Sample(
        ...
        label_factuality=1.0, label_tone=1.0, operator="gold", severity=0.0,
    )
...
def test_numeric_drift_none_without_grounded_number() -> None:
    g = Sample(source="s", trace="{}", article="No numbers here.",
               label_factuality=1.0, label_tone=1.0, operator="gold", severity=0.0)
...
def test_symmetric_positive_uses_paraphraser_but_stays_clean() -> None:
    ...
    assert out.label_factuality == 1.0 and out.label_tone == 1.0
