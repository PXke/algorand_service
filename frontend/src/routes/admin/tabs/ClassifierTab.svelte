<script lang="ts">
  ...
  // Phase 1) -- ArticleSourcesPanel owns loading/attach/remove; this tab just
  // needs the count for the badge next to Recompose.
  ...
  async function moveToDraft() {
    if (!current || pending || drafting) return
    const reviewId = String(current.review_id ?? '')
    if (!reviewId) return
    drafting = true
    error = null
    try {
      await admin.moveReviewToDraft({ review_id: reviewId })
      onmessage?.('Moved to draft — attach sources and reach out before publishing')
      reviews = reviews.filter((r) => itemKey(r) !== itemKey(current))
      clampIndex()
      resetItemState()
    } catch (e) {
...
<div class="tab admin-stack">
  ...
  {:else}
    <article class="admin-panel review-card">
      ...
      <ArticleSourcesPanel {admin} {articleId} onSourcesChange={(n) => (sourcesCount = n)} />
      ...
      <div class="decision-row">
        {#if current.review_id}
          ...
          {#if sourcesCount > 0}
            <span class="admin-chip sources-badge">
              Recompose · {sourcesCount} source{sourcesCount === 1 ? '' : 's'} attached
            </span>
          {/if}
        {/if}
        <div class="decision-actions">
          ...
          <button
            ...
            title="Park in draft instead of publishing — e.g. to ask the subject questions first"
            onclick={() => moveToDraft()}
          >
            {drafting ? 'Drafting…' : 'Move to draft'}
          </button>
