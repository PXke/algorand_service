...
def register_kyc_routes(app: Robyn) -> None:
    ...
    # Free, self-service: proves the requesting wallet's address to itself
    # for the caller to display before they sign. No payment gate — the
    # enrolled wallet is the one who benefits from being listed, not us.
    @app.get("/api/v1/kyc/consent-message")
    ...
    # Free enrollment: wallet-signed consent is the only gate. Computes trust
    # signals from the public indexer and stores/overwrites the wallet's
    # current KYC level — see EnrollmentService.
    @app.post("/api/v1/kyc/enroll")
    ...
    # check a wallet's KYC status. Charged whether or not the wallet is
    # enrolled (same as any paid lookup/search API) — the payout to the
    # enrolled wallet only fires on a hit, since there's no subject to reward
    # on a miss. See LookupService for the core rule (payout always goes to
    # the LOOKED-UP wallet, never the payer).
    @app.get("/api/v1/kyc/verify")
    ...
    # Admin-gated manual retry for a lookup whose payout failed (float too
    # low, opt-in missing, algod hiccup, confirm timeout) — see
    # kyc_lookup_events for which ones need it. Deliberately manual rather
    # than an automatic backoff sweep: ship simple first, automate later if
    # failures turn out to be common in practice.
    @app.post("/api/v1/admin/kyc/payouts/retry")
