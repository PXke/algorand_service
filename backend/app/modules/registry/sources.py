"""Service layer: map N sources (web domains, youtube channels, mail senders) onto one service, so multi-domain entities (algorand.co + algorand.com + the forum) feed a single weekly service-watch aggregate instead of composing near-duplicate articles per domain. Backend twin of the workers' ``app.modules.newspaper.service_sources`` — keep behaviours in sync."""
...
def domain_from_url(url: str) -> str:
    """Extract the normalized host domain from a URL."""
...
def add_web_source(service_id: str, *, domain: str, url: str) -> None:
    """Attach a web source to a service and claim the domain's reverse lookup (idempotent). source_id keys on the URL's HOST so one entity's subdomains stay distinct sources of ONE service — mirrors the workers' twin."""
...
def service_for_domain(domain: str) -> str:
    """Look up the service id that claims a domain's reverse lookup, if any."""
    from app.core.cassandra import get_cassandra_session
    from app.core.statements import ServiceSourceStmts
...
def merge_services(*, target_service_id: str, source_service_ids: list[str]) -> dict:
    """Fold whole services into ``target_service_id``: their sources move over, their domains re-point, and the emptied services are DISABLED in the registry (not deleted — audit trail + snapshots keep their history). The target service's next weekly poll then aggregates across all moved domains."""
    import contextlib
