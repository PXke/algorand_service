...
That is a real DDoS-amplification reduction, not just a speed win: even the
deliberately worst case (this module's own per-target rate limit) moves far
less data against any one target than a full-body fetch would.
...
transport hiccup) -- those are all valid, billable answers for a product
whose entire point is sometimes saying "it's down." It only lets a genuine
bug in this function itself propagate. This mirrors
`workers/app/modules/x402_probe/probe.py`'s `probe_url` contract ("never
raises for a per-URL failure"), not its code -- that module is
workers-side, scheduled, and only ever probes listed directory entries.
"""
...
_UA = (
    "PXke-x402-uptime-check/1 (+https://algorand.pxke.me/x402; "
    "caller-triggered on-demand reachability check, not a scheduled crawl)"
)
...
def check_target(
    ...
    *,
    ...
    (x402_scan, the x402 probe) requires one, because there is no body to
    slow-drip here.
    ...
    try:
        ...
        logger.exception("x402 uptime check: unexpected failure checking url=%r", url)
