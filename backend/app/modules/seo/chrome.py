...
from algorand_shared.design import palette
...
# instead of flashing to a different look the instant the SPA takes over.
# (They were hand-copied until 2026-07-27, and had drifted a full palette
# behind: the crumb separator was still the retired teal-era grey.)
#
# CSS braces are doubled for str.format; only {token} names interpolate.
_SSR_PALETTE = {name.replace("-", "_"): value for name, value in palette("light").items()}

SSR_CHROME_STYLE = (
    "#ssr-body{{max-width:880px;margin:0 auto;padding:24px 20px 32px;"
    "border-top:4px solid {accent};background:{surface};"
    "font:17px/1.55 Georgia,'Times New Roman',serif;color:{on_surface}}}"
    "#ssr-body img{{max-width:100%;height:auto}}"
    "#ssr-body a{{color:{primary};text-decoration:none}}"
    "#ssr-body a:hover{{text-decoration:underline}}"
    ".ssr-header{{border-bottom:1px solid {border};padding-bottom:16px;margin-bottom:20px;"
    "padding-top:20px}}"
    ".ssr-masthead{{margin-bottom:12px}}"
    ".ssr-brand{{margin:0;font:700 1.35em Georgia,'Times New Roman',serif}}"
    ".ssr-brand a{{color:inherit}}"
    ".ssr-tagline{{margin:4px 0 0;font:13px/1.4 system-ui,sans-serif;color:{muted}}}"
    ".ssr-nav ul{{display:flex;flex-wrap:wrap;gap:6px 18px;margin:0;padding:0;list-style:none;"
    "font:14px system-ui,sans-serif}}"
    ".ssr-nav a[aria-current=page]{{font-weight:700;color:{on_surface};text-decoration:underline}}"
    ".ssr-crumbs{{margin:0 0 18px;font:13px system-ui,sans-serif;color:{muted}}}"
    ".ssr-crumbs ol{{display:flex;flex-wrap:wrap;gap:4px 0;margin:0;padding:0;list-style:none}}"
    ".ssr-crumbs li+li::before{{content:'›';margin:0 8px;color:{subtle}}}"
    ".ssr-main{{margin-bottom:28px}}"
    ".ssr-footer{{border-top:1px solid {border};padding-top:24px;"
    ...
    ".ssr-footer-brand h2{{font-size:1.05em;letter-spacing:0;text-transform:none;color:{on_surface}}}"
    ".ssr-muted{{color:{muted};font-size:.92em}}"
    ".ssr-rights{{margin:0;font-size:12px;color:{muted}}}"
    ".ssr-tags{{font:14px system-ui,sans-serif;color:{muted};margin:0 0 14px}}"
    ".ssr-langs{{font:14px system-ui,sans-serif;color:{muted};margin:20px 0 0;"
    "padding-top:14px;border-top:1px solid {border}}}"
    ".ssr-langs [aria-current=true]{{font-weight:700;color:{on_surface}}}"
    ".ssr-related{{margin-top:28px;padding-top:20px;border-top:1px solid {border};"
    ...
    "#ssr-body .ssr-lead h1 a,#ssr-body .ssr-front li a{{color:{on_surface}}}"
    "#ssr-body .ssr-lead h1 a:hover,#ssr-body .ssr-front li a:hover{{color:{primary}}}"
