...
def admin_x402_storage_inspect(request: Request) -> Response | dict:
    ...
    logged with the admin wallet for audit.
    """
    denied = require_admin_wallet(request)
    if denied is not None:
        return denied
...
def admin_x402_storage_remove(request: Request) -> Response | dict:
    ...
    judgment call was made correctly -- it trusts the authenticated admin's
    own judgment, same as every other admin write in this file. Every call
    is logged with the admin wallet for audit.
    """
    denied = require_admin_wallet(request)
    if denied is not None:
        return denied
...
def register_admin_routes(app: Router) -> None:
    ...
    # x402 storage admin routes -- registered unconditionally, like every
    # other admin write in this file (protected by require_admin_wallet, not
    # a feature flag), but only meaningfully durable once
    # x402_storage_meta_store is off "memory" (see
    # app/modules/x402_storage/api/routes.py, falcon_main.py).
