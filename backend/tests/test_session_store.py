...
def test_pop_nonce_challenge_uses_atomic_getdel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    store = SessionStore()
    store.set_nonce_challenge("ADDR", '{"nonce":"n"}')
    assert store.pop_nonce_challenge("ADDR") == '{"nonce":"n"}'
    assert store.pop_nonce_challenge("ADDR") is None
    assert fake.getdel_calls == ["auth:nonce:ADDR", "auth:nonce:ADDR"]
    assert fake.store == {}
