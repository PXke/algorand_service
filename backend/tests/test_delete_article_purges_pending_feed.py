...
class _FakeSession:
    def __init__(self, *, published_at: datetime, pending_rows: list) -> None:
        self._published_at = published_at
        self._pending_rows = pending_rows
        self.pending_deletes: list[tuple] = []
        self.article_deletes: list[tuple] = []
    ...
    def execute(self, query: str, params: tuple = ()) -> _Result:
        ...
        if q.startswith("DELETE FROM algorand_platform.pending_feed_queue"):
            ...
            return _Result(None)
        if q.startswith("DELETE FROM algorand_platform.articles_by_id"):
            self.article_deletes.append(tuple(params))
            return _Result(None)
        return _Result(None)
...
def test_delete_article_purges_its_pending_feed_queue_row(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert fake.article_deletes == [(article_id,)]
