...
workers/tests/test_domain_from_url_parity.py; edit both together.
"""
...
# (input url, expected registrable domain) — mirror in the workers test.
VECTORS = [
...
def test_domain_from_url(url: str, expected: str) -> None:
    """Extracts the registrable eTLD+1 domain for each parity vector URL."""
    assert AdminCassandraStore._domain_from_url(url) == expected
