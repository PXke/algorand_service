...
class _StubFacilitator:
    """Canned /supported response, no network. verify()/settle() raise if
    called — the no-payment-header path must never reach either."""
...
def _stub_resource_server() -> x402ResourceServerSync:
    ...
    register_exact_avm_server(server, networks=ALGORAND_TESTNET_CAIP2)
    server.initialize()
    return server


def test_require_payment_returns_402_without_payment_header(monkeypatch):
...
def test_kyc_ping_round_trip_on_testnet():
    """Full 402 -> pay -> verify -> settle round trip against the real
    GoPlausible facilitator on TestNet. Written against x402-avm==2.0.2
    source (2026-07-13) but not yet executed — no funded TestNet account
    existed at write time. Re-check the lower-level calls here against the
    installed package first if this fails."""
    import base64
